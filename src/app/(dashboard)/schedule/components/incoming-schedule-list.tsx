...
// Mock data for incoming meetings (future dates)
const mockIncomingSchedules: Schedule[] = [
  {
    _id: '1',
...
export function IncomingScheduleList({ onEdit }: IncomingScheduleListProps) {
  const router = useRouter()
  const [searchTerm, setSearchTerm] = useState('')
  const [schedules] = useState<Schedule[]>(mockIncomingSchedules)
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center space-x-2">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search incoming meetings..."
            value={searchTerm}
        ...
        <Badge variant="secondary" className="ml-auto">
          {filteredSchedules.length} incoming meeting{filteredSchedules.length !== 1 ? 's' : ''}
        </Badge>
      </div>
            ...
            {filteredSchedules.length === 0 ? (
              <TableRow>
                <TableCell colSpan={8} className="h-24 text-center">
                  {searchTerm ? 'No incoming meetings found matching your search.' : 'No incoming meetings found.'}
                </TableCell>
              </TableRow>
            ) : (
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Showing {filteredSchedules.length} of {schedules.length} incoming meetings
        </p>
        <div className="flex items-center space-x-2">
